...
The server reads `PORT` and `HOST` from the environment. For local development it defaults to `127.0.0.1:4173`; container hosts should set `HOST=0.0.0.0`. Aggregate stats are stored at `.data/stats.json` by default and can be redirected with `CRASHPAD_STATS_PATH`.
...
The parser lives in `src/crashParser.js`. The browser harness lives in `index.html`, `src/app.js`, `src/stats.js`, and `src/styles.css`. The local server is `scripts/server.js`.

## Accessibility and Translation

The UI targets WCAG 2.2 AA checks for the main analyzer, privacy page, and stats page. Keep interactive controls keyboard reachable, preserve visible `:focus-visible` styling, and run an axe scan when changing layouts or generated report markup.
